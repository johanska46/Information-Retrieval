...
<table border="1">
	...
	foreach ($cosine as $key=>$values) {
		foreach ($values as $key1 => $value1) {
			...
			<td><?php 
			$sql = "SELECT t.co_dosen_pembimbing,d.nama_dosen from training as t, dosen_pembimbing as d where t.co_dosen_pembimbing = d.id_dosenP and id_doc= $key1 ";
			$dospem= mysqli_query($conn,$sql);
...
//Proses Prohabilitas dan COssine :
?>
	...
	foreach ($cosine as $key=>$values) {
		foreach ($values as $key1 => $value1) {
			...
			<!-- <tr>
				...
				<td> -->
					<?php 
					$sql = "SELECT co_dosen_pembimbing from training where id_doc= $key1 ";
					$dospem= mysqli_query($conn,$sql);
					while ($row=mysqli_fetch_array($dospem)) {
						//echo $row['co_dosen_pembimbing'];
						$nmadospem=$row['co_dosen_pembimbing'];
					}
					?>
			<!-- 	</td>
				...
				<td> -->
					<?php
					$sql = "SELECT count(co_dosen_pembimbing) as jumlah from training where co_dosen_pembimbing= $nmadospem group by co_dosen_pembimbing ";
					$dospem= mysqli_query($conn,$sql);
					while ($row=mysqli_fetch_array($dospem)) {
						$prob_doc= $value1 / ($value1 + $row['jumlah']);
						//echo $prob_doc;
...
<!-- 
		...
		<br>
		<h1>NBC co Dosen Pembimbing:</h1>
		<?php

// $NBC = array();
// $sql=" TRUNCATE `nbc`.`nbc_codospem_temp` ";
// $clear=mysqli_query($conn,$sql);
		$sql = "SELECT dospem from dosen_pembimbing as d, temp as t where t.dospem = d.id_dosenP and d.status= '2'  group by dospem";
		$result = mysqli_query($conn, $sql);
		while ($row=mysqli_fetch_array($result)) {
			$prob_nbc=0;
			$prob_nbc_dosen = 0;
...
//print_r($NBC);
		...
		<table border="1">
			...
			$nomor=1;
	// ksort($NBC);
			$sql = "SELECT * from nbc_dospem_temp as d, dosen_pembimbing as p where p.id_dosenP = d.id_dosen and p.status ='2' ORDER BY d.nbc DESC";
			$result = mysqli_query($conn, $sql);
			while ($row=mysqli_fetch_array($result)) {
				?>
				<tr>
